...
| **Cloud Extender (desktop side, AWS)** | `boto3` | Triggers the customer's own deployed scanner Lambda and reads back its DynamoDB results — never reads S3 object content directly. See §11. |
| **Cloud Extender (desktop side, GCP)** | `google-auth`, `google-cloud-storage`, `google-cloud-firestore`, `google-cloud-eventarc`, `requests` | Triggers the customer's own deployed Cloud Run service (OIDC-authenticated) and reads back its Firestore results — never reads GCS object content directly. See §15. |
| **Cloud Extender (in-account scanner, AWS)** | Same detection engine as above, deployed as a container image (Lambda + JVM for Tika) | Runs entirely inside the customer's own AWS account; see `cloud-extender/aws/`. Not part of the desktop app's own dependency set. |
| **Cloud Extender (in-account scanner, GCP)** | Same detection engine as above, deployed as a container image (Cloud Run + JVM for Tika) | Runs entirely inside the customer's own GCP project; see `cloud-extender/gcp/`. Not part of the desktop app's own dependency set. |
