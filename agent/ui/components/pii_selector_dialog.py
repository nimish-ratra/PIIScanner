"""
PII Selector, Viewer, and File List Modal Dialogs for CLAISSIFY
...
class PiiSelectorDialog(QDialog):
    ...
    def __init__(
        ...
        self.mode = mode
        if mode == "realtime":
            self.setWindowTitle("Configure Real-Time Detection Types - CLAISSIFY")
        else:
            self.setWindowTitle("Configure PII Detection Types - CLAISSIFY")
        self.resize(780, 600)
    ...
    def _build_ui(self) -> None:
        ...
        self.search_box.addAction(get_icon("search", "#94A3B8", 16), QLineEdit.LeadingPosition)
        ...
        all_btn.setIcon(get_icon("list-checks", "#ffffff", 14))
        ...
        for cat_name, cat_data in ENTITY_CATEGORIES.items():
            ...
            # QPushButton treats a lone "&" as a mnemonic marker and silently
            # strips it (unlike the QLabel this same badge string is also used
            # in elsewhere), so escape it here specifically.
            btn = QPushButton(f"{cat_data['badge']} ({count})".replace("&", "&&"), self)
            btn.setIcon(get_icon(cat_data.get("icon", "circle"), "#3f3f46", 14))
    ...
    def _on_save_clicked(self) -> None:
        if len(self.current_selection) == 0:
            QMessageBox.warning(
                ...
                "Tip: Click 'Recommended Defaults' to quickly select standard detection types, or click 'Select All'."
            )
            return
        self.accept()
    ...
    def _build_category_section(self, cat_name: str, cat_data: dict) -> QWidget:
        ...
        header = QHBoxLayout()
        cat_cb = QCheckBox(cat_data["badge"].replace("&", "&&"), box)
        cat_cb.setIcon(get_icon(cat_data.get("icon", "circle"), "#e2e8f0", 14))
    ...
    def _update_stats_label(self) -> None:
        ...
        if selected == 0:
            ...
            self.lbl_status.setText("At least 1 entity type must be selected to run scans.")
...
class PiiViewerDialog(QDialog):
    ...
    def __init__(
        ...
        super().__init__(parent)
        self.setWindowTitle(title or "Current Active PII Detection Types - CLAISSIFY")
        self.resize(700, 520)
        self.setModal(True)
    ...
    def _build_ui(self) -> None:
        ...
        if len(self.active_set) == 0:
            empty_box = QWidget(container)
            empty_vbox = QVBoxLayout(empty_box)
            empty_vbox.setAlignment(Qt.AlignCenter)
            empty_vbox.setSpacing(8)
            lbl_empty_icon = QLabel(empty_box)
            lbl_empty_icon.setAlignment(Qt.AlignCenter)
            lbl_empty_icon.setPixmap(get_icon("alert-triangle", "#f87171", 28).pixmap(28, 28))
            empty_vbox.addWidget(lbl_empty_icon)
            lbl_empty = QLabel(
                "No PII detection types are currently active.\n\n"
                "Click 'Edit PII Detection Types...' on the scan tab to enable detection types.",
                empty_box
            )
            lbl_empty.setStyleSheet("color: #f87171; font-size: 13px; font-weight: 600; padding: 0 30px 30px 30px; background: transparent;")
            lbl_empty.setAlignment(Qt.AlignCenter)
            empty_vbox.addWidget(lbl_empty)
            vbox_cards.addWidget(empty_box)
        else:
            for cat_name, cat_data in ENTITY_CATEGORIES.items():
                active_in_cat = [
                ...
                grp_hdr_row = QHBoxLayout()
                grp_hdr_row.setSpacing(6)
                lbl_hdr_icon = QLabel(grp)
                lbl_hdr_icon.setFixedSize(14, 14)
                lbl_hdr_icon.setPixmap(get_icon(cat_data.get("icon", "circle"), "#38bdf8", 14).pixmap(14, 14))
                grp_hdr_row.addWidget(lbl_hdr_icon)
                grp_hdr = QLabel(f"{cat_data['badge']} ({len(active_in_cat)} active)", grp)
                grp_hdr.setStyleSheet("font-size: 13px; font-weight: 700; color: #38bdf8; background: transparent;")
                grp_hdr_row.addWidget(grp_hdr)
                grp_hdr_row.addStretch()
                grp_vbox.addLayout(grp_hdr_row)

                for ent, desc in active_in_cat:
                    row = QHBoxLayout()
                    row.setSpacing(4)
                    lbl_check_icon = QLabel(grp)
                    lbl_check_icon.setFixedSize(14, 14)
                    lbl_check_icon.setPixmap(get_icon("check", "#34d399", 14).pixmap(14, 14))
                    row.addWidget(lbl_check_icon)
                    lbl_name = QLabel(ent, grp)
                    lbl_name.setStyleSheet("font-weight: 600; font-size: 12px; color: #f1f5f9; min-width: 170px;")
            ...
            if additional_active:
                ...
                lbl_hdr_icon = QLabel(grp)
                lbl_hdr_icon.setFixedSize(14, 14)
                lbl_hdr_icon.setPixmap(get_icon("settings", "#38bdf8", 14).pixmap(14, 14))
                grp_hdr_row.addWidget(lbl_hdr_icon)
                grp_hdr = QLabel(f"Additional ({len(additional_active)} active)", grp)
                grp_hdr.setStyleSheet("font-size: 13px; font-weight: 700; color: #38bdf8; background: transparent;")
                grp_hdr_row.addWidget(grp_hdr)
                grp_hdr_row.addStretch()
                grp_vbox.addLayout(grp_hdr_row)
...
class FileViewerDialog(QDialog):
    ...
    def __init__(
        ...
        super().__init__(parent)
        self.setWindowTitle(f"Discovered Documents ({len(files)}) - CLAISSIFY")
        self.resize(720, 480)
        self.setModal(True)
