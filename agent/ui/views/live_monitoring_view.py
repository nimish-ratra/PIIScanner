"""
CLAISSIFY - Live Pre-Save Monitoring & Enforcement View
...
from ui.components.enforcement_details_dialog import EnforcementEventDetailsDialog, get_source_icon, get_source_icon_name
...
class LiveMonitoringView(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.lbl_service_badge = StatusBadge(
            "dot", "#94a3b8", "CHECKING SERVICE...",
            bg_color="#1e293b", size=10, parent=header_card,
        )
        ...
        self.btn_start_svc.setIcon(get_icon("play", "#ffffff", 16))
        ...
        self.btn_stop_svc.setIcon(get_icon("square", "#ffffff", 16))
        ...
        self.btn_probe.setIcon(get_icon("flask-conical", "#ffffff", 16))
        ...
        btn_open_quarantine.setIcon(get_icon("folder", "#3f3f46", 16))
        ...
        self.btn_inspect_ev = QPushButton("Inspect Event...", self)
        self.btn_inspect_ev.setIcon(get_icon("eye", "#3f3f46", 14))
    ...
    def _create_entity_kpi_card(self) -> QFrame:
        ...
        btn_edit = QPushButton("Edit", card)
        btn_edit.setIcon(get_icon("settings", "#3f3f46", 14))
    ...
    def _apply_service_status(self, running: bool, data: Dict[str, Any]) -> None:
        ...
        if running:
            self.lbl_service_badge.set_state(
                "shield-check", "#34d399", "REAL-TIME PROTECTION ACTIVE (Port 47821)",
                bg_color="rgba(16, 185, 129, 0.2)", border_color="#10b981",
            )
            ...
            self.lbl_service_badge.set_state(
                "shield", "#f87171", "REAL-TIME PROTECTION STOPPED",
                bg_color="rgba(239, 68, 68, 0.2)", border_color="#ef4444",
            )
    ...
    def _on_start_service(self) -> None:
        self.btn_start_svc.setEnabled(False)
        self.lbl_service_badge.set_state("clock", "#fbbf24", "STARTING SERVICE...", bg_color="rgba(245, 158, 11, 0.2)", border_color="#f59e0b")
        success = ServiceController.start(policy_manager.api_port)
        self._check_service_status()
        if success:
            QMessageBox.information(
                self, "Live Protection Started",
                f"CLAISSIFY real-time protection is now active on 127.0.0.1:{policy_manager.api_port}.\n"
    ...
    def _on_stop_service(self) -> None:
        self.btn_stop_svc.setEnabled(False)
        self.lbl_service_badge.set_state("clock", "#fbbf24", "STOPPING SERVICE...", bg_color="rgba(245, 158, 11, 0.2)", border_color="#f59e0b")
    ...
    def _apply_fetched_events(self, events: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            for row_idx, ev in enumerate(self.enforcement_events):
                ...
                item_src.setIcon(get_icon(get_source_icon_name(src), "#64748B", 14))
                ...
                item_tier = QTableWidgetItem(tier_name)
                item_tier.setIcon(QIcon(dot_pixmap(tier_color, 10)))
                item_tier.setTextAlignment(Qt.AlignCenter)
                item_tier.setForeground(QColor(tier_color))
                ...
                if override_bool:
                    item_ov = QTableWidgetItem(reason if reason else "Yes")
                    item_ov.setIcon(get_icon("alert-triangle", "#fbbf24", 14))
