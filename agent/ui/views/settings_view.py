...
class SettingsView(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.tabs.addTab(tab_general, get_icon("settings", "#64748B", 16), "General && Scan Defaults")

        # Tab 2: Enforcement Policy
        tab_enforcement = self._create_enforcement_tab()
        self.tabs.addTab(tab_enforcement, get_icon("shield", "#64748B", 16), "Real-Time Enforcement Policy")

        # Tab 3: Watermarking & Safety Net
        tab_watermarking = self._create_watermarking_tab()
        self.tabs.addTab(tab_watermarking, get_icon("tag", "#64748B", 16), "Watermarking && Data Protection")
    ...
    def _create_general_tab(self) -> QWidget:
        ...
        # NOT scroll.setStyleSheet(...) - an inline stylesheet on this
        # intermediate ancestor blocks the app-wide theme's QPushButton/
        # #primaryButton/etc. rules from cascading to every widget inside
        # this tab.
        # setFrameShape gets the same borderless look via a real property;
        # the transparent background already comes from theme.py's global
        # `QScrollArea { background: transparent; border: none; }` rule.
    ...
    def _create_enforcement_tab(self) -> QWidget:
        scroll = QScrollArea(self)
        scroll.setWidgetResizable(True)
        scroll.setFrameShape(QFrame.NoFrame)  # see _create_general_tab() for why not setStyleSheet
        ...
        notice_box.setObjectName("phase2NoticeBox")
        # Scoped to #phase2NoticeBox, not a bare "QFrame" type selector -
        # QLabel is itself a QFrame subclass, so a generic type selector set
        # here would cascade to every child QLabel below too, drawing a
        # second border/background tightly around each one's own text.
        notice_box.setStyleSheet("""
            QFrame#phase2NoticeBox {
        ...
        notice_title_row = QHBoxLayout()
        notice_title_row.setSpacing(6)
        lbl_notice_icon = QLabel(notice_box)
        lbl_notice_icon.setFixedSize(15, 15)
        lbl_notice_icon.setStyleSheet("background: transparent; border: none;")
        lbl_notice_icon.setPixmap(get_icon("shield", "#e2e8f0", 15).pixmap(15, 15))
        notice_title_row.addWidget(lbl_notice_icon)
        lbl_notice_title = QLabel("Phase 2 Real-Time Save Enforcement", notice_box)
        lbl_notice_title.setStyleSheet("color: #e2e8f0; font-size: 12px; font-weight: 700; background: transparent; border: none;")
        notice_title_row.addWidget(lbl_notice_title)
        notice_title_row.addStretch()
        ...
        notice_lbl.setStyleSheet("color: #e2e8f0; font-size: 12px; line-height: 1.4; background: transparent; border: none;")
        ...
        self.btn_rt_view_entities = QPushButton("View Current Real-Time Types", grp_rt_types)
        self.btn_rt_view_entities.setIcon(get_icon("eye", "#f1f5f9", 16))
        ...
        self.btn_rt_edit_entities = QPushButton("Edit Real-Time Types...", grp_rt_types)
        self.btn_rt_edit_entities.setIcon(get_icon("sliders-horizontal", "#ffffff", 16))
        ...
        for row_idx, tier in enumerate(tier_list):
            ...
            tier_color = meta.get("color_hex", meta.get("color", "#94a3b8"))
            badge_row = QWidget(grp_tiers)
            badge_row_layout = QHBoxLayout(badge_row)
            badge_row_layout.setContentsMargins(0, 0, 0, 0)
            badge_row_layout.setSpacing(6)
            lbl_badge_dot = QLabel(badge_row)
            lbl_badge_dot.setFixedSize(10, 10)
            lbl_badge_dot.setPixmap(dot_pixmap(tier_color, 10))
            badge_row_layout.addWidget(lbl_badge_dot)
            lbl_badge = QLabel(tier.value, badge_row)
            lbl_badge.setStyleSheet(f"font-weight: 700; color: {tier_color}; font-size: 13px; background: transparent;")
            badge_row_layout.addWidget(lbl_badge)
            badge_row_layout.addStretch()

            combo_action = QComboBox(grp_tiers)
            for act_val, act_label in action_options:
                combo_action.addItem(act_label, act_val)

            grid_tiers.addWidget(badge_row, row_idx, 0)
        ...
        lbl_loopback_icon = QLabel(grp_failsafe)
        lbl_loopback_icon.setFixedSize(14, 14)
        lbl_loopback_icon.setPixmap(get_icon("lock", "#34d399", 14).pixmap(14, 14))
        port_box.addWidget(lbl_loopback_icon)
        lbl_loopback = QLabel("Strictly Loopback (No External Network Interface)", grp_failsafe)
        lbl_loopback.setStyleSheet("color: #34d399; font-size: 12px; font-weight: 600; background: transparent;")
        ...
        self.lbl_svc_status = StatusBadge("circle", "#94a3b8", "Not Tested", bg_color="#1e293b", size=12, parent=grp_diag)
        diag_box.addWidget(self.lbl_svc_status)
        diag_box.addSpacing(16)

        self.btn_test_svc = QPushButton("Test Service Probe", grp_diag)
        self.btn_test_svc.setIcon(get_icon("search", "#3f3f46", 16))
        ...
        badge_word_stat = StatusBadge(
            "check-circle" if addin_status_word else "alert-triangle",
            "#34d399" if addin_status_word else "#f59e0b",
            f"Word: {'Registered' if addin_status_word else 'Not Registered'}",
            parent=grp_diag,
        )
        badge_excel_stat = StatusBadge(
            "check-circle" if addin_status_excel else "alert-triangle",
            "#34d399" if addin_status_excel else "#f59e0b",
            f"Excel: {'Registered' if addin_status_excel else 'Not Registered'}",
            parent=grp_diag,
        )
        addin_box.addWidget(badge_word_stat)
        addin_box.addWidget(badge_excel_stat)
    ...
    def _create_watermarking_tab(self) -> QWidget:
        ...
        self.lbl_auto_notice = QLabel(
            "Notice: Automatic watermarking isn't available yet in this version. "
        ...
        self.edit_template.setText(config_manager.get(
            "watermark_template",
            "CONFIDENTIAL — Classified by CLAISSIFY — {tier} — {date} — Do Not Distribute"
        ))
        ...
        btn_add_excl.setIcon(get_icon("plus", "#3f3f46", 16))
        ...
        btn_rem_excl.setIcon(get_icon("minus", "#3f3f46", 16))
        ...
        btn_reset_excl.setIcon(get_icon("rotate-ccw", "#3f3f46", 16))
        ...
        btn_open_bkp.setIcon(get_icon("folder-open", "#3f3f46", 16))
        btn_open_bkp.clicked.connect(lambda: reveal_in_explorer(str(get_watermark_backup_dir())))
        bkp_action_row.addWidget(btn_open_bkp)

        btn_prune = QPushButton("Prune Expired Backups Now", grp_bkp)
        btn_prune.setIcon(get_icon("trash-2", "#3f3f46", 16))
    ...
    def _on_reset_watermarking_settings(self) -> None:
        ...
        self.edit_template.setText("CONFIDENTIAL — Classified by CLAISSIFY — {tier} — {date} — Do Not Distribute")
    ...
    def _on_rt_view_entities(self) -> None:
        dlg = PiiViewerDialog(config_manager.realtime_selected_entities, self, title="Active Real-Time PII Entities - CLAISSIFY")
        dlg.exec()
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=1.5) as resp:
                ...
                if data.get("status") == "healthy":
                    self.lbl_svc_status.set_state(
                        "check-circle", "#34d399", "Service Healthy (127.0.0.1)",
                        bg_color="rgba(16, 185, 129, 0.2)",
                    )
                else:
                    self.lbl_svc_status.set_state(
                        "alert-triangle", "#fbbf24", "Degraded Status",
                        bg_color="rgba(245, 158, 11, 0.2)",
                    )
        except Exception:
            self.lbl_svc_status.set_state(
                "x-circle", "#f87171", "Service Offline / Not Running",
