...
from PySide6.QtGui import QAction, QClipboard, QGuiApplication, QColor, QIcon
...
class ResultsView(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        for _tier_name, _hex in (
            ...
            self.combo_classification.addItem(QIcon(dot_pixmap(_hex, 10)), _tier_name)
        ...
        self.btn_toggle_reveal = QPushButton("Reveal Full Values", self)
        self.btn_toggle_reveal.setIcon(get_icon("eye", "#94A3B8", 16))
        ...
        self.btn_quarantine.setIcon(get_icon("shield", "#ffffff", 16))
        ...
        self.btn_review_watermarks = QPushButton("Watermark Candidates", self)
        self.btn_review_watermarks.setIcon(get_icon("shield", "#3f3f46", 16))
        ...
        self.btn_undo_watermark.setIcon(get_icon("rotate-ccw", "#3f3f46", 16))
    ...
    def _populate_table(self, items: List[Dict[str, Any]]) -> None:
        ...
        for row_idx, item in enumerate(items):
            ...
            if wm_status == "applied":
                wm_text, wm_color, wm_icon = "Watermarked", "#34d399", "tag"
            elif wm_status == "reverted":
                wm_text, wm_color, wm_icon = "Reverted", "#94a3b8", "rotate-ccw"
            elif wm_status == "failed":
                wm_text, wm_color, wm_icon = "Failed", "#f87171", "x-circle"
            elif wm_status == "pending":
                wm_text, wm_color, wm_icon = "Pending", "#fbbf24", "clock"
            else:
                wm_text, wm_color = "—", "#64748b"

            item_wm = QTableWidgetItem(wm_text)
            item_wm.setTextAlignment(Qt.AlignCenter)
            item_wm.setForeground(QColor(wm_color))
            if wm_icon:
                item_wm.setIcon(get_icon(wm_icon, wm_color, 14))
    ...
    def _on_toggle_reveal(self, checked: bool) -> None:
        ...
        if checked:
            ...
            self.btn_toggle_reveal.setIcon(get_icon("lock", "#fca5a5", 16))
            if header_item:
                header_item.setText("Value (Full / Unmasked)")
        else:
            self.btn_toggle_reveal.setText("Reveal Full Values")
            self.btn_toggle_reveal.setIcon(get_icon("eye", "#94A3B8", 16))
            if header_item:
    ...
    def _apply_filters(self) -> None:
        ...
        for f in self.findings:
            ...
            if selected_class != "All Classifications":
                target_tier = selected_class.strip().lower()
    ...
    def _show_context_menu(self, pos) -> None:
        ...
        action_inspect = QAction(get_icon("search", "#18181b", 14), "Inspect Finding Details...", self)
...
class FindingDetailsDialog(QDialog):
    ...
    def __init__(self, item: Dict[str, Any], parent: Optional[QWidget] = None):
        super().__init__(parent)
        self.setWindowTitle("Finding Inspector - CLAISSIFY")
        self.resize(680, 440)
        self.setModal(True)
        self.item = item
        self._build_ui()
    ...
    def _build_ui(self) -> None:
        ...
        btn_open_folder.setIcon(get_icon("folder-open", "#94a3b8", 14))
