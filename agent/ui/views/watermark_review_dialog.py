...
class WatermarkReviewDialog(QDialog):
    ...
    def __init__(self, candidates: List[Dict[str, Any]], parent: Optional[QWidget] = None):
        ...
        self.setWindowTitle("Watermark Candidates Review — CLAISSIFY")
        self.resize(920, 580)
        self.setModal(True)
    ...
    def _build_ui(self) -> None:
        ...
        lbl_title_icon.setFixedSize(18, 18)
        lbl_title_icon.setPixmap(get_icon("shield", "#f8fafc", 18).pixmap(18, 18))
        title_row.addWidget(lbl_title_icon)
        title = QLabel("Watermark Candidates Review", self)
        title.setStyleSheet("font-size: 18px; font-weight: 800; color: #f8fafc; background: transparent;")
        title_row.addWidget(title)
        title_row.addStretch()
        vbox_title.addLayout(title_row)
    ...
    def _populate_table(self) -> None:
        ...
        for row_idx, item in enumerate(self.candidates):
            ...
            badge_tier = StatusBadge(
                "dot", tier_color, tier_name,
                bg_color="rgba(255, 255, 255, 0.05)", border_color=tier_color,
            )
            self.table.setCellWidget(row_idx, 1, badge_tier)
