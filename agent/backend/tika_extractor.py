    ...
    def extract_text(self, filepath: str) -> Tuple[str, Optional[str]]:
        ...
        try:
            ...
            if content:
                ...
                # Tika parsed the file but produced no text - this covers both a
                # genuinely blank/image-only document AND a corrupted/malformed
                # file that Tika silently gave up on without raising. Since we
                # can't tell those apart, surface it as an error (needs manual
                # review) rather than silently classifying it "no PII found" -
                # a scanner that can't read a file must not report it as clean.
                return "", "No extractable text (file may be blank, image-only without OCR, or corrupted/unsupported)"
